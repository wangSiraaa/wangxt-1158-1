...
router.get('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({ success: true, data: { ...measurement, velocityPoints, waterLevel, auditTrails } })
  } catch (error) {
...
router.post('/:id/submit', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (wl && changeRateAbs > 0.5) {
      needRemeasure = true
      warning = `水位变化率 ${wl.changeRate}m/h（绝对值 ${changeRateAbs}m/h）超过 0.5m/h 阈值，已标记需要复测`
    }

    const now = new Date().toISOString()
    const { operator = '未知', operatorRole = 'station', comment, force } = req.body
